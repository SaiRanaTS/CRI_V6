...
def cpa(ov_spd, ov_heading, ov_x, ov_y, tv_spd, tv_heading, tv_x, tv_y):
    ...
    Dc1 = (CRI_FunExe.CRI_call(ov_spd, tv_spd, ov_x_N, ov_y_N, tv_x_N, tv_y_N, ov_heading, tv_heading))
    # print(CRI_u)
    # return 1852*Dc1[0], 3600*Dc1[2],cpa, Dc1[1],Dc1[3]
    timm = 0
    pck = (Dc1[0], Dc1[2], cpa, 0.9*Dc1[1], Dc1[3], Dc1[4], timm,ov_spd,tv_spd,dcpa,tcpa,Dc1[5],Dc1[6],Dc1[7],Dc1[8],Dc1[9],Dc1[10],Dc1[11],Dc1[12],Dc1[13],Dc1[14],Dc1[15],Dc1[16],Dc1[17],Dc1[18],Dc1[19],Dc1[20],Dc1[21],Dc1[22],Dc1[23],Dc1[23])
    return pck
...
def update(frame):
    ...
    tv1_x.append(str2float(tv1_data.row_values(frame)[4]) - origin_x)
    tv1_y.append(str2float(tv1_data.row_values(frame)[5]) - origin_y)
    tv1_heading.append(str2float((tv1_data.row_values(frame)[1])))
    tv1_spd.append(str2float(tv1_data.row_values(frame)[3]))
    tv1_mk_rtt = transforms.Affine2D().rotate_deg(-tv1_heading[-1])
    r11 = RotatedRect( ov_x[-1], ov_y[-1],[-0.005, -0.02], [0.005, -0.02], [0.005, 0.01], [0, 0.02], [-0.005, 0.01],ov_heading[-1])
    ...
    T_alot = trgs_ang.trags_calc(tv1_spd[-1],ov_spd[-1],tv1_x[-1],tv1_y[-1],ov_x[-1],ov_y[-1],tv1_heading[-1],ov_heading[-1])
    dec1 = deci_sup.dec_sup(T_alot,Dbtwn1,CRI_tv1,alp_ot1,tv1_heading[-1])

    # -------------------------------------- Domain Implementation-------------------------------------------------------
    domain_termTS1 = domain.domain(tsvv1, tsan1, Ts1_ln)
    ...
    plt_t1_cir = ax.scatter(tv1_x[-1], tv1_y[-1], marker=t1_crk, facecolors='none', edgecolors='r', s=skl_T1 ** 3.2, alpha=0.26, linestyle='dashed')
    ...
    tv2_x.append(str2float(tv2_data.row_values(frame)[4]) - origin_x)
    tv2_y.append(str2float(tv2_data.row_values(frame)[5]) - origin_y)
    tv2_heading.append(str2float((tv2_data.row_values(frame)[1])))
    tv2_spd.append(str2float(tv2_data.row_values(frame)[3]))
    ...
    # -------------------------------------- Decision Support Function Call---------------------------------------------
    T_alot2 = trgs_ang.trags_calc(tv2_spd[-1], ov_spd[-1], tv2_x[-1], tv2_y[-1], ov_x[-1], ov_y[-1], tv2_heading[-1],ov_heading[-1])
    dec2 = deci_sup.dec_sup(T_alot2, Dbtwn2, CRI_tv2, alp_ot2, tv2_heading[-1])
    ...
    plt_tv2_txt = plt.text(tv2_x[-1] + 100, tv2_y[-1], f'T2: Stavangerfjord \n CRI = {round(CRI_tv2, 3)}\nDCPA = {round(dcpa_tv2, 1)}m,\n TCPA = {-round(tcpa_tv2, 3)}s ', fontsize=10,color='white')

    # --------------------------------------------- Domain Implementation-----------------------------------------------
    domain_termTS2 = domain.domain(tsvv2, tsan2, Ts2_ln)
    ...
    def_Domain2 = Path([[-TS2D2, -TS2D3], [TS2D1, -TS2D3], [TS2D1, TS2D4], [0, TS2D5], [-TS2D2, TS2D4], [0, 0]],
                       [Path.MOVETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.CLOSEPOLY])
    ...
    tv3_x.append(str2float(tv3_data.row_values(frame)[4]) - origin_x)
    tv3_y.append(str2float(tv3_data.row_values(frame)[5]) - origin_y)
    tv3_heading.append(str2float((tv3_data.row_values(frame)[1])))
    tv3_spd.append(str2float(tv3_data.row_values(frame)[3]))
    ...
    # -------------------------------------- Decision Support Function Call---------------------------------------------
    T_alot3 = trgs_ang.trags_calc(tv3_spd[-1], ov_spd[-1], tv3_x[-1], tv3_y[-1], ov_x[-1], ov_y[-1], tv3_heading[-1], ov_heading[-1])
    dec3 = deci_sup.dec_sup(T_alot3, Dbtwn3, CRI_tv3, alp_ot3, tv3_heading[-1])
    ...
    plt_tv3_txt = plt.text(tv3_x[-1] + 100, tv3_y[-1], f'T3: UNI \n CRI = {round(CRI_tv3, 3)}\nDCPA = {round(dcpa_tv3, 1)}m,\n TCPA = {-round(tcpa_tv3, 3)}s ', fontsize=10,color='white')

    # -------------------------------------- Domain Implementation-------------------------------------------------------
    domain_termTS3 = domain.domain(tsvv3, tsan3, Ts3_ln)
    ...
    def_Domain3 = Path([[-TS3D2, -TS3D3], [TS3D1, -TS3D3], [TS3D1, TS3D4], [0, TS3D5], [-TS3D2, TS3D4], [0, 0]],
                       [Path.MOVETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.CLOSEPOLY])
    ...
    tv4_x.append(str2float(tv4_data.row_values(frame)[4]) - origin_x)
    tv4_y.append(str2float(tv4_data.row_values(frame)[5]) - origin_y)
    tv4_heading.append(str2float((tv4_data.row_values(frame)[1])))
    tv4_spd.append(str2float(tv4_data.row_values(frame)[3]))
    ...
    # -------------------------------------- Decision Support Function Call---------------------------------------------
    T_alot4 = trgs_ang.trags_calc(tv4_spd[-1], ov_spd[-1], tv4_x[-1], tv4_y[-1], ov_x[-1], ov_y[-1], tv4_heading[-1],
                                  ov_heading[-1])
    dec4 = deci_sup.dec_sup(T_alot4, Dbtwn4, CRI_tv4, alp_ot4, tv4_heading[-1])
    ...
    plt_tv4_txt = plt.text(tv4_x[-1] + 100, tv4_y[-1], f'T4: Frigat 1 \n CRI = {round(CRI_tv4, 3)}\nDCPA = {round(dcpa_tv4, 1)}m,\n TCPA = {-round(tcpa_tv4, 3)}s ', fontsize=10,color='white')

    # ------------------------------------------ Domain Implementation---------------------------------------------------
    domain_termTS4 = domain.domain(tsvv4, tsan4, Ts4_ln)
    ...
    def_Domain4 = Path([[-TS4D2, -TS4D3], [TS4D1, -TS4D3], [TS4D1, TS4D4], [0, TS4D5], [-TS4D2, TS4D4], [0, 0]],
                       [Path.MOVETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.CLOSEPOLY])
    ...
    tv5_x.append(str2float(tv5_data.row_values(frame)[4]) - origin_x)
    tv5_y.append(str2float(tv5_data.row_values(frame)[5]) - origin_y)
    tv5_heading.append(str2float((tv5_data.row_values(frame)[1])))
    tv5_spd.append(str2float(tv5_data.row_values(frame)[3]))
    ...
    # -------------------------------------- Decision Support Function Call---------------------------------------------
    T_alot5 = trgs_ang.trags_calc(tv5_spd[-1], ov_spd[-1], tv5_x[-1], tv4_y[-1], ov_x[-1], ov_y[-1], tv5_heading[-1],
                                  ov_heading[-1])
    dec5 = deci_sup.dec_sup(T_alot5, Dbtwn5, CRI_tv4, alp_ot5, tv5_heading[-1])
    ...
    plt_tv5_txt = plt.text(tv5_x[-1] + 100, tv5_y[-1], f'T5: Frigat 2 \n CRI = {round(CRI_tv5, 3)}\nDCPA = {round(dcpa_tv5, 1)}m,\n TCPA = {-round(tcpa_tv5, 3)}s ', fontsize=10,color='white')

    # -------------------------------------- Domain Implementation-------------------------------------------------------
    domain_termTS5 = domain.domain(tsvv5, tsan5, Ts5_ln)
    ...
    def_Domain5 = Path([[-TS5D2, -TS5D3], [TS5D1, -TS5D3], [TS5D1, TS5D4], [0, TS5D5], [-TS5D2, TS5D4], [0, 0]],
                       [Path.MOVETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.LINETO, Path.CLOSEPOLY])
    ...
    plt_t5_cir = ax.scatter(tv5_x[-1], tv5_y[-1], marker=t5_crk, facecolors='none', edgecolors='yellow', s= skl_T5 ** 3.2,
                            alpha=0.1,linestyle='dashed')
...
#-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-
    ...
    if CRI_tv1 > 0.6 and Dbtwn1 < 0.4:
        plt_Dec1 = plt.text(4000, 3400, f'T1: Hannara entered the CAZ \n {dec1[0]} \n {dec1[1]}\n {dec1[2]}\n {dec1[3]}\n {dec1[4]}',
                            fontsize=10, color='white')

    elif CRI_tv2 > 0.6 and Dbtwn2 < 0.4:
        plt_Dec2 = plt.text(4000, 3400,f'T2: Stavangerfjord entered the CAZ\n {dec2[0]} \n {dec2[1]}\n {dec2[2]}\n {dec2[3]}\n {dec2[4]}',
                            fontsize=10, color='white')
    elif CRI_tv3 > 0.6 and Dbtwn3 < 0.4:
        plt_Dec3 = plt.text(4000, 3400, f'T3: UNI entered the CAZ\n {dec3[0]} \n {dec3[1]}\n {dec3[2]}\n {dec3[3]}\n {dec3[4]}',
                            fontsize=10, color='white')
    elif CRI_tv4 > 0.6 and Dbtwn4 < 0.4:
        plt_Dec4 = plt.text(4000, 3400, f'T4: Frigat 1 entered the CAZ\n {dec4[0]} \n {dec4[1]}\n {dec4[2]}\n {dec4[3]}\n {dec4[4]}',
                            fontsize=10, color='white')
    elif CRI_tv5 > 0.6 and Dbtwn5 < 0.4:
        plt_Dec5 = plt.text(4000, 3400, f'T5: Frigat 2 entered the CAZ\n {dec5[0]} \n {dec5[1]}\n {dec5[2]}\n {dec5[3]}\n {dec5[4]}',
                            fontsize=10, color='white')
    else:
...
# Vessel Information Plot
    ...
    Vsl1_Info = plt.text(3700,1300,f'T1:Hannara\nDCPA = {round(dcpa_tv1, 1)}NM\nTCPA = {-round(tcpa_tv1 * 3600, 3)}s\nCRI = {round(CRI_tv1, 3)}\nVelocity = {round(tv1_spd[-1], 1)} Kn',fontsize=10, color='white')
